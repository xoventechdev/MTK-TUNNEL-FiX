...
public class AuthFormHandler extends UserDialog implements DialogInterface.OnClickListener, DialogInterface.OnDismissListener, SettingsConstants {
	...
	private LinearLayout newDropdown(final LibOpenConnect.FormOpt opt, int selection) {
		...
		ArrayAdapter<String> adapter = new ArrayAdapter<String>(mContext, android.R.layout.simple_spinner_item, choiceList);
	...
	private void saveAndStore() {
		for (LibOpenConnect.FormOpt opt : mForm.opts) {
			...
			switch (opt.type) {
				case LibOpenConnect.OC_FORM_OPT_TEXT: {
					opt.value = ConfigUtil.getInstance(mContext).getSecureString(USERNAME_KEY);
					break;
				}
				case LibOpenConnect.OC_FORM_OPT_PASSWORD: {
					String s = ConfigUtil.getInstance(mContext).getSecureString(PASSWORD_KEY);
					opt.value = s;
					break;
				}
				case LibOpenConnect.OC_FORM_OPT_SELECT:
	...
	public Object earlyReturn() {
		...
		for (LibOpenConnect.FormOpt opt : mForm.opts) {
			...
			switch (opt.type) {
				case LibOpenConnect.OC_FORM_OPT_PASSWORD:
					String password = ConfigUtil.getInstance(mContext).getSecureString(PASSWORD_KEY);
					if (password.isEmpty()) {
						return null;
					}
					opt.value = password;
					break;
				case LibOpenConnect.OC_FORM_OPT_TEXT:
					String username = ConfigUtil.getInstance(mContext).getSecureString(USERNAME_KEY);
					if (username.isEmpty()) {
						return null;
					}
					opt.value = username;
	...
	private final LinearLayout.LayoutParams fillWidth = new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT);
