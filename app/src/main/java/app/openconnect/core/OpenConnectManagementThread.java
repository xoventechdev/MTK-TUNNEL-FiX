...
public class OpenConnectManagementThread implements Runnable, OpenVPNManagement, SettingsConstants {
	...
	private final OcsVpnProfile mProfile;
	private final OCService mOpenVPNService;
	...
	private final HashMap<String,Boolean> mAcceptedCerts = new HashMap<String,Boolean>();
	private final HashMap<String,Boolean> mRejectedCerts = new HashMap<String,Boolean>();
    ...
    public OpenConnectManagementThread(Context context, OcsVpnProfile profile, OCService openVpnService) {
		...
		putStringPref("reported_os","android");
	}
    ...
    private void log(String msg) {
    	mOpenVPNService.addLogInfo(msg);
    }

    private boolean isCertAccepted(String hash) {
    	if (mAcceptedCerts.containsKey(hash) || getStringPref("ACCEPTED-CERT-" + hash).equals("true")) {
    		return true;
    	}
    	return false;
    }
	...
	private class AndroidOC extends LibOpenConnect {
		...
		public int onProcessAuthForm(LibOpenConnect.AuthForm authForm) {
			...
			if (response == OC_FORM_RESULT_OK) {
				...
				log("AUTH: requesting authgroup change " + (mAuthgroupSet ? "(interactive)" : "(non-interactive)"));
				mAuthgroupSet = true;
			} else {
		...
		public void onProgress(int level, String msg) {
			mOpenVPNService.addLogInfo( "LIB: " , msg.trim());
		}
	...
	private String prefToTempFile(String prefName, boolean isExecutable) throws IOException {
		...
		if (prefData.startsWith(OcsVpnProfile.INLINE_TAG)) {
	...
	private boolean setPreferences() {
		...
		mServerAddr = mConfig.getSecureString(SERVER_KEY) + ":" + mConfig.getSecureString(SERVER_PORT_KEY);
		mOC.setHTTPProxy(mConfig.getProxyAddress());
		mOC.setXMLPost(true);
	...
	private void errorAlert(String message) {
		mOpenVPNService.addLogInfo(mContext.getString(R.string.error_connection_failed),message);
	}

	private void errorAlert() {
		log(mContext.getString(R.string.error_cant_connect, mOC.getHostname()));
	}
	...
	private void extractBinaries() {
		...
		try {
			...
			writeCertOrScript(mFilesDir + "/curl", "#!/system/bin/sh\nexec " + run_pie + curl_bin + " \"$@\"\n", true);
		} catch (IOException e) {
