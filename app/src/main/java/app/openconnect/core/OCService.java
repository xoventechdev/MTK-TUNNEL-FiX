...
public class OCService extends VpnService {
	...
	public OcsVpnProfile profile;
	public LibOpenConnect.IPInfo ipInfo;
	public String serverName;
	public Date startTime;

	private ocsDeviceStateReceiver mDeviceStateReceiver;
	...
	private void killVPNThread(boolean joinThread) {
		if (doStopVPN() && joinThread) {
			try {
				...
				addLogInfo("OpenConnect thread did not exit");
			}
		}
	}
	...
	private void registerDeviceStateReceiver(OpenVPNManagement management) {
		...
		filter.addAction(ocsDeviceStateReceiver.PREF_CHANGED);
		filter.addAction(Intent.ACTION_SCREEN_OFF);
		filter.addAction(Intent.ACTION_SCREEN_ON);
		mDeviceStateReceiver = new ocsDeviceStateReceiver(management, mPrefs);
	...
	private synchronized void registerKeepAlive() {
		...
		try {
			...
			addLogInfo("server DNS IP is bogus, falling back to " + DNSServer + " for KeepAlive", e.toString());
		}
		...
		idle = idle * 4 / 10;
		addLogInfo("calculated KeepAlive interval: " + idle + " seconds");
	...
	private void unregisterReceivers() {
		try {
			...
			addLogInfo("can't unregister DeviceStateReceiver", iae.toString());
		}
		...
		try {
			...
			addLogInfo("can't unregister KeepAlive", iae.toString());
		}
	}
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		if (intent == null) {
			addLogInfo("OpenVpnService started with null intent");
			stopSelf();
	...
	private void wakeUpActivity() {
		mHandler.post(() -> {
            ...
            if (mConnectionState == OpenConnectManagementThread.STATE_CONNECTED && mKeepAlive == null) {
                registerKeepAlive();
            }
        });
	}
	...
	public synchronized void threadDone() {
		...
		addLogInfo("VPN thread has terminated");
		mVPN = null;
		mHandler.post(() -> {
            if (stopSelfResult(startId) == false) {
				addLogInfo("not stopping service due to startId mismatch");
            } else {
                unregisterReceivers();
            }
        });
	...
	public void setConnectionState(int state) {
		switch (state){
			...
			case OpenConnectManagementThread.STATE_DISCONNECTED:
				startService(new Intent(OCService.this, HarlieService.class).setAction(HarlieService.STOP_SERVICE).putExtra("stateSTOP_SERVICE",getResources().getString(R.string.state_disconnected)));
				break;
			case OpenConnectManagementThread.STATE_CONNECTED:
				startTime = new Date();
				hLogStatus.updateStateString(hLogStatus.VPN_CONNECTED, getResources().getString(R.string.state_connected));
				break;
		}
	...
	public String getReconnectName() {
		OcsVpnProfile p = ProfileManager.get(mUUID);
	...
	public void addLogInfo(String msg1,String msg2){
		hLogStatus.logInfo(String.format("%s %s", msg1,msg2));
	}
