...
public class TransportConnection {
    ...
    StatisticGraphData.DataTransferStats _upDateBytes;
    ...
    public TransportConnection(InputStream is, OutputStream os, SecureRandom rnd) {
        ...
        this.rnd = rnd;
        this._upDateBytes = StatisticGraphData.getStatisticData().getDataTransferStats();
    }
    ...
    public void sendMessage(byte[] message, int off, int len, int padd) throws IOException {
        ...
        cos.flush();
        _upDateBytes.addBytesSent(bytesSent);
        if (log.isEnabled()) {
    ...
    public int receiveMessage(byte buffer[], int off, int len) throws IOException {
        ...
        _upDateBytes.addBytesReceived(payload_length);
        return payload_length;
    }
}
